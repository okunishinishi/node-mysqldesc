...
MysqlDescriber.prototype = {
    ...
    _getRelations: function(data, table){
      var relationships = [];
      var references = this._parseTableKeyColumnUsageData(data);

      Object.keys(references).forEach(function (key) {
          let refs = references[key];
          if (refs.hasOwnProperty('REFERENCED_TABLE_NAME')){
            relationships.push(refs['REFERENCED_TABLE_NAME'] + "." + refs['REFERENCED_COLUMN_NAME']);
          }
      });
